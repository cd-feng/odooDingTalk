...
class DinDinWorkRecord(models.Model):
    ...
    def send_record(self):
        """发送待办事项函数"""
        self.ensure_one()
        url = self.env['dingding.parameter'].search([('key', '=', 'workrecord_add')]).value
    ...
    def send_message_to_emp(self):
        ...
        data = {
            ...
            'userid_list': self.emp_id.ding_id,
            'msg': {
                "msgtype": "oa",
                "oa": {
    ...
    def get_workrecord(self):
        ...
        din_ids = self.env['hr.employee'].search_read([('ding_id', '!=', '')], fields=['ding_id'])
        offset = 0  # 分页游标
        limit = 50  # 分页大小
        for emp in din_ids:
            while True:
                result = self.get_workrecord_url(emp.get('ding_id'), offset, limit)
                try:
                    if 'list' in result:
    ...
    def get_workrecord_url(self, user_id, offset, limit):
        url = self.env['dingding.parameter'].search([('key', '=', 'workrecord_getbyuserid')]).value
        token = self.env['dingding.parameter'].search([('key', '=', 'token')]).value
        data = {
            'userid': user_id,
            'offset': offset,
    ...
    def record_update(self):
        ...
        for res in self:
            logging.info("待办更新")
            url = self.env['dingding.parameter'].search([('key', '=', 'workrecord_update')]).value
            token = self.env['dingding.parameter'].search([('key', '=', 'token')]).value
            data = {
                'userid': res.emp_id.ding_id,
...
class GetUserDingDingWorkRecord(models.TransientModel):
    ...
    def get_user_work_record(self):
        ...
        """
        self.ensure_one()
        ding_id = self.env['hr.employee'].search_read([('user_id', '=', self.env.user.id)], fields=['ding_id'])
        if len(ding_id) > 1:
