...
class GetProcessInstance(models.TransientModel):
    ...
    oa_madel = fields.Many2one(comodel_name='dingtalk.approval.template', string=u'审批模型')
    ...
    def get_process_list(self):
        ...
        dac = self.env['dingtalk.approval.control'].search([('template_id', '=', self.oa_madel.id)], limit=1)
        if not dac:
