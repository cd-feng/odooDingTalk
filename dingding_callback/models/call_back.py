...
class DinDinCallback(models.Model):
    ...
    def register_call_back(self):
        ...
        logging.info(">>>注册事件...")
        for res in self:
            url = self.env['dingding.parameter'].search([('key', '=', 'register_call_back')]).value
    ...
    def update_call_back(self):
        ...
        """
        for res in self:
            url = self.env['dingding.parameter'].search([('key', '=', 'update_call_back')]).value
            token = self.env['dingding.parameter'].search([('key', '=', 'token')]).value
            call_list = list()
            for call in res.call_ids:
                call_list.append(call.value)
    ...
    def delete_call_back(self, call_token):
        logging.info(">>>删除事件...")
        url = self.env['dingding.parameter'].search([('key', '=', 'delete_call_back')]).value
        token = self.env['dingding.parameter'].search([('key', '=', 'token')]).value
        data = {
    ...
    def get_all_call_back(self):
        ...
        :return:
        """
        url = self.env['dingding.parameter'].search([('key', '=', 'get_call_back')]).value
        token = self.env['dingding.parameter'].search([('key', '=', 'token')]).value
        try:
