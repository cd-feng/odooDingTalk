...
class CallBack(Home, http.Controller):
    ...
    def callback_users(self, **kw):
        ...
        call_back, corp_id = self.get_bash_attr()
        msg = self.encrypt_result(json_str.get('encrypt'), call_back.aes_key, corp_id)
        ...
        # 返回加密结果
        return self.result_success(call_back.aes_key, call_back.token, corp_id)
    ...
    def encrypt_result(self, encrypt, encode_aes_key, corp_id):
        """
        解密钉钉回调返回的值
        :param encrypt:
        :param encode_aes_key:
