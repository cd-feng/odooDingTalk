...
class DinDinCalendarEvent(models.Model):
    ...
    def create(self, values):
        ...
        auto_calendar_event = self.env['ir.config_parameter'].sudo().get_param('dingding_base.auto_calendar_event')
        if auto_calendar_event:
            values['dingtalk_calendar_id'] = self.create_dingding_calendar(values)
            self.sudo().message_post(body=u"已同步上传至钉钉日程", message_type='notification')
        return super(DinDinCalendarEvent, self).create(values)

    @api.model
    def create_dingding_calendar(self, val):
        """
        创建钉钉日程
        :param val:
        :return:
        """
        url = self.env['dingding.parameter'].search([('key', '=', 'calendar_create')]).value
        ...
        userids = list()
        userids.append(employee.ding_id)
        data = {
            'create_vo': {
                ...
                'creator_userid': employee[0].ding_id if employee[0].ding_id else '',  # 创建人uid
