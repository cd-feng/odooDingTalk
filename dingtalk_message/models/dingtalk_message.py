...
class MessageDataSet(DataSet):
    ...
    def call_button(self, model, method, args, kwargs):
        call_result = super(MessageDataSet, self).call_button(model, method, args, kwargs)
    ...
    def call_kw(self, model, method, args, kwargs, path=None):
        """ 处理常规操作[新建、更新、删除]触发消息 """
        call_result = super(MessageDataSet, self).call_kw(model, method, args, kwargs, path=path)
        ir_model = request.env['ir.model'].sudo().search([('model', '=', model)], limit=1)
        if not request.session.uid:
            request.uid = request.env.ref('base.public_user').id
        else:
        ...
        user = request.env['res.users'].search([('id', '=', user_id)])
        domain = [('model_id', '=', ir_model.id), ('company_id', '=', user.company_id.id),
                  ('state', '=', 'open'), ('msg_opportunity', '=', 'normal')]
        msg_configs = request.env['dingtalk.message.config'].sudo().search(domain)
        for msg_config in msg_configs:
            if method == 'create' and msg_config.message_timing == 'save' and args[0]:
                # 获取当前单据的id
                res_name = args[0].get('name')
                now_model = request.env[model].sudo().search([('name', '=', res_name)])
                message_tool = request.env['dingtalk.message.tool']
                threading.Thread(target=message_tool.send_notice_message,
                                 args=(msg_config.id, model, now_model.id, user.company_id)).start()
            elif (method == 'unlink' and msg_config.message_timing == 'unlink') or \
                    (method == 'write' and msg_config.message_timing == 'write'):
                # 获取当前单据的id
                if args[0]:
                    res_id = args[0][0]
                else:
