...
class UpdateDingtalkEmployeeAvatar(models.TransientModel):
    ...
    def on_update(self):
        ...
        t = threading.Thread(target=self.update_dingtalk_employee_avatar, args=(user_id, self.company_ids.ids))
        t.daemon = True
        t.start()
        return self.env.user.notify_success(message="系统正在后台替换员工头像，请耐心等待处理完成...")

    def update_dingtalk_employee_avatar(self, user_id, company_ids):
        """
        执行替换操作
        :return:
        """
