...
class ReissueApplication(models.Model):
    ...
    def summit_approval(self):
        ...
        # 表单参数
        fcv_list = list()
        fcv_list.append({'name': '补签人', 'value': self.emp_id.name})
        fcv_list.append({'name': '补签日期', 'value': str(self.reissue_date)})
        fcv_list.append({'name': '补签类型', 'value': self.reissue_type})
        fcv_list.append({'name': '补签事由', 'value': self.reissue_text})
        fcv_list.append({'name': '申请事由', 'value': self.reason_leave})
        # 发送单据信息至钉钉并接受审批实例id
    ...
    def create(self, values):
        values['process_code'] = self.env['ir.sequence'].sudo().next_by_code('oa.reissue.application.code')
