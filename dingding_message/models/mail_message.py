...
class MailMessage(models.Model):
    ...
    def create(self, values):
        ...
        results = template.get_template_by_model_and_type(
            model, values.get('message_type'))
        if results:
            for result in results:
                # 备注消息
                if values.get('message_type') == 'comment':
                    partner = self.env['res.partner'].sudo().search(
                        [('id', '=', values.get('author_id'))])
                    base_url = "{}/dingding/auto/login/in".format(
                    ...
                    # 指定单据发送人
                        ...
                        for field in result.field_ids:
                            sql = "SELECT {} FROM {} WHERE id = {}".format(
                                field.name, values['model'].replace('.', '_'), values['res_id'])
                            logging.info(sql)
                    ...
                    # 指定群机器人
                        ...
                        self.env['dingding.robot'].sudo().send_robot_message(
                            result.rotbot_ids, msg)
        return message
...
class DingDingMessageTemplate(models.Model):
    ...
    def generate_message_text(self, model_name, body_html, res_id):
        ...
        :return:
        """
        result = self.env['mail.template'].sudo()._render_template(
            body_html, model_name, res_id)
        return result
