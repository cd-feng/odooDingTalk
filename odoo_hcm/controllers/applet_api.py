...
class AppletApiController(Controller):
    ...
    @route('/api/applet/enterprise/dynamic/by/id', type='http', auth='none', methods=['get', 'post'], csrf=False)
    def get_enterprise_dynamic_by_id(self, **kw):
        """
        获取企业动态数据详情，根据id获取对应的记录
        :param kw:
        :return:
        """
        params = request.params.copy()
        ...
        if not res_id:
            return json.dumps({'state': False, 'msg': '记录ID不存在'})
        dynamics = request.env['applet.enterprise.dynamic'].sudo().search([('id', '=', res_id)], limit=1)
        result_data = dict()
        for dynamic in dynamics:
            tag_list = list()
            for tag in dynamic.tag_ids:
        ...
        return json.dumps({'state': True, 'msg': '查询成功', 'data': result_data})
