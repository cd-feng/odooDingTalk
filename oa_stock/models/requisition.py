...
class OaStockRequisition(models.Model):
    ...
    def summit_approval(self):
        ...
        fcv_list.append({'name': '转出位置', 'value': self.out_location.name if self.out_location else ''})
        fcv_list.append({'name': '转入仓库', 'value': self.ing_warehouse.name if self.ing_warehouse else ''})
        fcv_list.append({'name': '转入位置', 'value': self.ing_location.name if self.ing_location else ''})
