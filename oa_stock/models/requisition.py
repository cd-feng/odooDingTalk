...
class OaStockRequisition(models.Model):
    ...
    def summit_approval(self):
        ...
        fcv_list.append({'name': '联系人', 'value': self.contact_id.name})
        fcv_list.append({'name': '联系电话', 'value': self.contact_phone})
        fcv_list.append({'name': '收货地址', 'value': self.shipping_address})
        ...
        fcv_list.append({'name': '转出仓库', 'value': self.out_warehouse.name})
        fcv_list.append({'name': '转入仓库', 'value': self.ing_warehouse.name})
        fcv_list.append({'name': '调拨方式', 'value': self.transfer_method.name})
        fcv_list.append({'name': '银行渠道', 'value': self.bank_channel.name})
        fcv_list.append({'name': '货运方式', 'value': self.delivery_method.name})
        fcv_list.append({'name': '货运单号', 'value': self.delivery_number})
        fcv_line = list()
