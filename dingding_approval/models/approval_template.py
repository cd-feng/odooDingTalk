...
class DinDinApprovalTemplate(models.Model):
    ...
    def get_template(self):
        """获取审批模板"""
        logging.info(">>>获取审批模板...")
        url = self.env['dingding.parameter'].search([('key', '=', 'process_listbyuserid')]).value
    ...
    def get_get_template_number_by_user(self):
        ...
        if emp and emp.ding_id:
            url = self.env['dingding.parameter'].search([('key', '=', 'process_gettodonum')]).value
            token = self.env['dingding.parameter'].search([('key', '=', 'token')]).value
            data = {
                'userid': emp.ding_id,
            }
    ...
    def get_processinstance(self, pid, pcode):
        ...
        :return:
        """
        url = self.env['dingding.parameter'].search([('key', '=', 'processinstance_get')]).value
        token = self.env['dingding.parameter'].search([('key', '=', 'token')]).value
        data = {
            ...
            if result.get('errcode') == 0:
                ...
                if temp:
                    ...
                    if appro:
                        ...
                        if not oa_model:
                            # 获取发起人
                            emp = self.env['hr.employee'].sudo().search([('ding_id', '=', process_instance.get("originator_userid"))])
                            data = {
