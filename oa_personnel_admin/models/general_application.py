...
class GeneralApplication(models.Model):
    ...
    reason_leave = fields.Text(string=u'申请事由', required=True)
    ...
    def summit_approval(self):
        ...
        fcv_list.append({'name': '申请部门', 'value': self.dept_id.name})
    ...
    def create(self, values):
        values['process_code'] = self.env['ir.sequence'].sudo().next_by_code('oa.general.application.code')
        return super(GeneralApplication, self).create(values)
